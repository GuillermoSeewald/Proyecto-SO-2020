...
int main() {
    ...
    printf("--------------------------------------------------------------------------------\n");
    sleep(2);
    mainQueueId = getQueue(getKey(KEY_ID_MAIN_QUEUE));
    ordersQueueId = getQueue(getKey(KEY_ID_ORDERS_QUEUE));
    initQueue();
    tableHeader();
    childProcess();
...
void removeQueue() {
    msgctl(mainQueueId, IPC_RMID, (struct msqid_ds*) NULL);
    msgctl(ordersQueueId, IPC_RMID, (struct msqid_ds*) NULL);
}
