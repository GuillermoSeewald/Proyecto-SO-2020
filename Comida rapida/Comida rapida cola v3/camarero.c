...
void waiterWork() {
    ...
    while (1) {
        msgrcv(queueId, &msg, SIZE_MSG, -4, IPC_CREAT);
        if (msg.id == TYPE_ORDER_MEAT_MENU) {
            ...
            if (msg.id == TYPE_ORDER_VEG_MENU) {
                tableRow(columnNumber, messages.takingVegeOrderMessage);
                msgrcv(queueId, &msg, SIZE_MSG, TYPE_VEG_QUEUE_FULL, 0);
                ...
                tableRow(columnNumber, "DEVUELVO EL MENSAJE");
                msgsnd(queueId, &msg, SIZE_MSG, IPC_NOWAIT);
            }
        }
    }
